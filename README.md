...
### Pick and Place<br>
  ...
  3. 하나의 물체만을 생성했음에도 불구하고,제대로 들어올리지 못하는 문제가 발생함. 이후 전체 과정을 구간별로 나누어 수집 진행
     전체 pick and place : 1000개
     lift 구간 : 500개
     move 구간 : 500개
     lower 구간 : 500개
     retreat 구간 : 500개
  ...
  4. move하는 과정 중 물체를 놓치는 문제가 발생하여 데이터 수집 시에만 pitch를 고정시켜 안정성을 강화함
     전체 pick and place : 2000개
     lift 구간 : 1000개
     move 구간 : 1000개
     lower 구간 : 1000개
     retreat 구간 : 1000개
